...
### Ask user if they are on the APP or DB tier
read -p  "Enter either 'ap' or 'db' for the tier you want to refresh : " TIER
echo ""
printf "You have chosen to refresh the $TIER tier"
sleep 2s

### Ask user for 2 digits for a lab refresh
read -n 2 -p  "Enter the 2 digit lab you wish to refresh: " LABN
echo ""
printf "You have chosen to refresh LAB $LABN"
sleep 2s
...
if [ $TIER="db" ]; then CURRENTDB="LABORADB$LABN"
  ...
  ### APP ONLY STUFFS
elif [ $TIER="ap" ]; then CURRENTAPP="LABORAAPP$LABN"
