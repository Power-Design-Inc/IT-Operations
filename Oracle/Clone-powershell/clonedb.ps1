...
$dbVMuuid = Get-Vm -Server $vcsa02 -name ("*DB" + $labn) | ForEach-Object { (Get-View $_.Id).config.uuid }  
Start-Sleep -s 1
$appVMuuid
$dbVMuuid
...
$vmDBname = Get-Vm -Server $vcsa02 -name ("*DB" + $labn) | ForEach-Object { (Get-View $_.Id).name }
Start-Sleep -s 1
$vmAPPname
$vmDBname
...
$vmdbid = get-vm -Server $vcsa02 -name ("*DB" + $labn) | ForEach-Object { (Get-View $_.Id).moref.value }
Start-Sleep -s 1
$vmappid
$vmdbid
...
$vmdbds = get-vm -Server $vcsa02 -name ("*DB" + $labn) | ForEach-Object { (Get-View $_.Id).datastore.value }
Start-Sleep -s 1
$vmappds
$vmdbds
...
Remove-Datastore -Datastore ('LAB' + $labn + 'DB1'), ('LAB' + $labn + 'DB2'), ('LAB' + $labn + 'DB3'), ('LAB' + $labn + 'DB4'), ('LAB' + $labn + 'DB5'), ('LAB' + $labn + 'DB6') -VMHost 'pdi-esx23.powerdesigninc.us' -Confirm:$false
Start-Sleep -s 1
...
# 
Get-Datastore ('LAB' + $labn + 'DB1'), ('LAB' + $labn + 'DB2'), ('LAB' + $labn + 'DB3'), ('LAB' + $labn + 'DB4'), ('LAB' + $labn + 'DB5'), ('LAB' + $labn + 'DB6'), ('LABAPP' + $labn) | Move-datastore -Destination ('LAB' + $labn)
Start-Sleep -s 2
...
$dbMoref = get-vm -Server $vcsa02 -name ("*DB" + $labn) | ForEach-Object { (Get-View $_.Id).moref }
$spec = New-Object VMware.Vim.VirtualMachineConfigSpec
$spec.CpuAllocation = New-Object VMware.Vim.ResourceAllocationInfo
$spec.CpuAllocation.Shares = New-Object VMware.Vim.SharesInfo
$spec.CpuAllocation.Shares.Shares = 2000
